...
const std::wstring ENCODER_ID_CGA_REPORT =
    L"com.esri.prt.core.CGAReportEncoder";
...
bool isPRTInitialized() { return (bool)prtCtx; }

void shutdownPRT() { prtCtx.reset(); }
...
GeneratedModel::GeneratedModel(const size_t &initShapeIdx,
                               const std::vector<std::vector<double>> &vert,
                               const std::vector<std::vector<uint32_t>> &face,
                               const py::dict &rep)
    : mInitialShapeIndex(initShapeIdx), mVertices(vert), mFaces(face),
      mReport(rep) {}

namespace {

void extractMainShapeAttributes(const py::dict &shapeAttr,
                                std::wstring &ruleFile, std::wstring &startRule,
                                int32_t &seed, std::wstring &shapeName,
                                pcu::AttributeMapPtr &convertShapeAttr) {
  convertShapeAttr = pcu::createAttributeMapFromPythonDict(
      shapeAttr,
      *(pcu::AttributeMapBuilderPtr(prt::AttributeMapBuilder::create())));
  if (convertShapeAttr) {
...
ModelGenerator::ModelGenerator(const std::string &initShapePath) {
  ...
  mCache = (pcu::CachePtr)prt::CacheObject::create(
      prt::CacheObject::CACHE_TYPE_DEFAULT);
  ...
  if (!pcu::toFileURI(initShapePath).empty()) {
    LOG_DBG << "trying to read initial shape geometry from "
            << pcu::toFileURI(initShapePath) << std::endl;
    const prt::Status s = isb->resolveGeometry(
        pcu::toUTF16FromOSNarrow(pcu::toFileURI(initShapePath)).c_str(),
        mResolveMap.get(), mCache.get());
    if (s != prt::STATUS_OK) {
      LOG_ERR << "could not resolve geometry from "
              << pcu::toFileURI(initShapePath);
      mValid = false;
    }
  } else {
...
ModelGenerator::ModelGenerator(const std::vector<InitialShape> &myGeo) {
  ...
  for (size_t ind = 0; ind < myGeo.size(); ind++) {
    ...
    if (isb->setGeometry(myGeo[ind].getVertices(), myGeo[ind].getVertexCount(),
                         myGeo[ind].getIndices(), myGeo[ind].getIndexCount(),
                         myGeo[ind].getFaceCounts(),
...
void ModelGenerator::setAndCreateInitialShape(
    const std::vector<py::dict> &shapesAttr,
  ...
  for (size_t ind = 0; ind < mInitialShapesBuilders.size(); ind++) {
    ...
    extractMainShapeAttributes(shapeAttr, ruleF, startR, randomS, shapeN,
                               convertedShapeAttr[ind]);

    mInitialShapesBuilders[ind]->setAttributes(
        ruleF.c_str(), startR.c_str(), randomS, shapeN.c_str(),
...
void ModelGenerator::initializeEncoderData(const std::wstring &encName,
                                           const py::dict &encOpt) {
  mEncodersNames.clear();
  mEncodersOptionsPtr.clear();

  mEncodersNames.push_back(encName);
  const pcu::AttributeMapPtr encOptions{
      pcu::createAttributeMapFromPythonDict(encOpt, *mEncoderBuilder)};
  mEncodersOptionsPtr.push_back(
      createValidatedOptions(encName.c_str(), encOptions));

  if (encName != ENCODER_ID_PYTHON) {
    mEncodersNames.push_back(ENCODER_ID_CGA_REPORT);
    mEncodersNames.push_back(ENCODER_ID_CGA_PRINT);

    const pcu::AttributeMapBuilderPtr optionsBuilder{
        prt::AttributeMapBuilder::create()};
    optionsBuilder->setString(ENCODER_OPT_NAME, FILE_CGA_REPORT);
    const pcu::AttributeMapPtr reportOptions{
        optionsBuilder->createAttributeMapAndReset()};
    const pcu::AttributeMapPtr printOptions{
        optionsBuilder->createAttributeMapAndReset()};

    mEncodersOptionsPtr.push_back(
        createValidatedOptions(ENCODER_ID_CGA_REPORT, reportOptions));
    mEncodersOptionsPtr.push_back(
        createValidatedOptions(ENCODER_ID_CGA_PRINT, printOptions));
  }
}

void ModelGenerator::getRawEncoderDataPointers(
    std::vector<const wchar_t *> &allEnc,
  ...
  if (mEncodersNames[0] == ENCODER_ID_PYTHON) {
    ...
    allEnc.push_back(
        mEncodersNames[1]
            .c_str()); // an encoder to redirect CGA report to CGAReport.txt
    allEnc.push_back(
        mEncodersNames[2]
            .c_str()); // redirects CGA print output to the callback
...
std::vector<GeneratedModel>
ModelGenerator::generateModel(const std::vector<py::dict> &shapeAttributes,
  ...
  if ((shapeAttributes.size() != 1) &&
      (shapeAttributes.size() <
       mInitialShapesBuilders.size())) { // if one shape attribute dictionary,
  ...
  try {
    ...
    if (!rulePackagePath.empty()) {
      ...
      try {
        mResolveMap.reset(prt::createResolveMap(
            pcu::toUTF16FromUTF8(u8rpkURI).c_str(), nullptr, &status));
      } catch (std::exception &e) {
        pybind11::print("CAUGHT EXCEPTION:", e.what());
      }

      if (mResolveMap && (status == prt::STATUS_OK)) {
        LOG_DBG << "resolve map = " << pcu::objectToXML(mResolveMap.get())
                << std::endl;
      } else {
        LOG_ERR << "getting resolve map from '" << rulePackagePath
                << "' failed, aborting.";
        return {};
      }
    }
    ...
    std::vector<const prt::InitialShape *> initialShapes(
        mInitialShapesBuilders.size());
    std::vector<pcu::InitialShapePtr> initialShapePtrs(
        mInitialShapesBuilders.size());
    std::vector<pcu::AttributeMapPtr> convertedShapeAttrVec(
        mInitialShapesBuilders.size());
    setAndCreateInitialShape(shapeAttributes, initialShapes, initialShapePtrs,
                             convertedShapeAttrVec);
    ...
    if (mEncodersNames[0] == ENCODER_ID_PYTHON) {
      ...
      pcu::PyCallbacksPtr foc{
          std::make_unique<PyCallbacks>(mInitialShapesBuilders.size())};

      // Generate
      const prt::Status genStat = prt::generate(
          initialShapes.data(), initialShapes.size(), nullptr, encoders.data(),
          encoders.size(), encodersOptions.data(), foc.get(), mCache.get(),
          nullptr);

      if (genStat != prt::STATUS_OK) {
        LOG_ERR << "prt::generate() failed with status: '"
                << prt::getStatusDescription(genStat) << "' (" << genStat
                << ")";
        return {};
      }

      for (size_t idx = 0; idx < mInitialShapesBuilders.size(); idx++) {
        newGeneratedGeo.emplace_back(
            idx, convertVerticesIntoPythonStyle(foc->getVertices(idx)),
            foc->getFaces(idx), foc->getReport(idx));
      }
    } else {
      const std::filesystem::path outputPath =
          geometryEncoderOptions["outputPath"].cast<std::string>();
      ...
      if (std::filesystem::is_directory(outputPath) &&
        ...
        foc.reset(
            prt::FileOutputCallbacks::create(outputPath.wstring().c_str()));
      } else {
...
std::vector<GeneratedModel> ModelGenerator::generateAnotherModel(
    const std::vector<py::dict> &shapeAttributes) {
  if (!mResolveMap) {
...
PYBIND11_MODULE(pyprt, m) {
  py::bind_vector<std::vector<GeneratedModel>>(m, "GeneratedModelVector",
  ...
  py::class_<ModelGenerator>(m, "ModelGenerator")
      ...
      .def("generate_model", &ModelGenerator::generateModel,
           py::arg("shapeAttributes"), py::arg("rulePackagePath"),
           py::arg("geometryEncoderName"), py::arg("geometryEncoderOptions"))
      .def("generate_model", &ModelGenerator::generateAnotherModel,
           py::arg("shapeAttributes"));
