...
const std::wstring LEVELS[] = {L"trace",   L"debug", L"info",
...
template <prt::LogLevel L> struct StreamLogger : public Logger {
  ...
  template <typename T> StreamLogger<L> &operator<<(const T &x) {
    mOut << x;
    return *this;
  }
...
template <prt::LogLevel L> struct PRTLogger : public Logger {
  ...
  template <typename T> PRTLogger<L> &operator<<(const T &x) {
    wstr << x;
    return *this;
  }
...
template <prt::LogLevel L> using LT = PRTLogger<L>;
