...
public:
  ...
  virtual char *toXML(char *result, size_t *resultSize,
                      prt::Status *stat = 0) const {
...
struct PRTContext {
  PRTContext(prt::LogLevel minimalLogLevel) {
    ...
    const std::filesystem::path moduleRoot =
        pcu::getModuleDirectory().parent_path();
    ...
    mPRTHandle.reset(
        prt::init(extPaths.data(), extPaths.size(), minimalLogLevel));
  }
  ...
  explicit operator bool() const { return (bool)mPRTHandle; }
...
public:
  GeneratedModel(const size_t &initialShapeIdx,
...
public:
  ...
  generateModel(const std::vector<py::dict> &shapeAttributes,
                const std::string &rulePackagePath,
                const std::wstring &geometryEncoderName,
                const py::dict &geometryEcoderOptions);
  std::vector<GeneratedModel>
  generateAnotherModel(const std::vector<py::dict> &shapeAttributes);
...
private:
  ...
  void
  setAndCreateInitialShape(const std::vector<py::dict> &shapeAttr,
  ...
  void
  getRawEncoderDataPointers(std::vector<const wchar_t *> &allEnc,
